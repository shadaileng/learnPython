...
`functools`模块的`partial`方法,接收一个函数以及器默认参数键值,然后将默认参数给固定住(也就是设置默认值),最后返回一个新的函数
...
- 使用模块可以避免函数名和变量名冲突
- 为了避免模块名冲突,使用目录来组织模块,这个目录结构称之为包
- 包目录下必须有一个名为`__init__.py`的文件,否则这个目录会被当作普通文件夹
- 访问包目录下的模块格式是: `包名.模块名`
...
- 第`1`行和第`2`行是标准注释,
    - 第`1`行在类`Unix`系统中指定了运行环境
    - 第`2`行指定源文件编码格式为`UTF-8`
- 任何模块的第一个字符串都是文档注释,赋值给变量`__doc__`
- `__author__`变量表示作者
- `import`语句导入依赖的模块
- 直接运行模块文件的时候,`__name__`会被解析器赋值为`__main__`,`if`后面的条件成立,执行之后的语句.但是如果导入改模块的话,添加不成立,则不会执行之后的语句
...
- 使用关键字`class`定义一个类,`class`之后是类名,括号内是器父类,所有的类都是`object`类的子类
